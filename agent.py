"""The agent, and the one tool it can call.

The platform handles speech, turn-taking and the phone line. It does not decide
what to say: `llm=` points back at this process, so every turn is answered by
`brain.py`. The tool is served from here too, on the same tunnel.
"""
...
def build(base_url: str) -> VoiceAgent:
    ...
    return VoiceAgent(
        name="Weather Line",
        voice=VOICE,
        # Replaced on every turn by brain.py, so this only has to be true.
        ...
        llm=LlmConfigRequest(base_url=f"{base_url}/v1", model="weather-brain", api_key=SECRET),
    )
